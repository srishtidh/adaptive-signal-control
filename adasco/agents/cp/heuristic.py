...
class HeuristicAgent(BaseAgent):
    ...
    def plan(self, sensor_data, curr_phase, curr_phase_duration, curr_time):
        ...
        # logger.debug('{}: curr_time={}'.format(self.id, curr_time))
        # logger.debug('{}: curr_phase={}'.format(self.id, curr_phase))
        # logger.debug('{}: curr_phase_duration={}'.format(self.id, curr_phase_duration))
        # logger.debug('{}: Reached decision point'.format(self.id))
        ...
        if cluster_count > 0:
            ...
            for base_inflow in inflows:
                plan = self.scheduler.get_plan_for_sample(base_inflow, curr_phase, curr_phase_duration)

                print('Plan:')
                pprint.pprint(plan)

                delay_over_all_samples = 0
                for inflow, weight in zip(inflows, weights):
                    # TODO: Get cluster departures
                    delay = self.scheduler.schedule_inflow_according_to_plan(inflow, plan)
                    print('Delay over sample = {}'.format(delay))
                    delay_over_all_samples += delay * weight

                print('Total delay over all samples = {}'.format(delay_over_all_samples))
                   
                if delay_over_all_samples <= lowest_delay:
                    print('Updated lowest delay = {}'.format(delay_over_all_samples))
                    lowest_delay = delay_over_all_samples
                    best_plan = plan

            time_cp = timeit.default_timer() - start_cp

            print('Took {} to compute'.format(time_cp))

            curr_phase_start, curr_phase_end = best_plan[(0, curr_phase)]
            
            extension = int(round(curr_phase_end))
            ...
            logger.debug('{}: Scheduled extension = {}'.format(self.id, extension))

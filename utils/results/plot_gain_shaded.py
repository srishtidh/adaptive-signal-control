...
colours = {900: 'orange', 1350: 'royalblue', 1800: 'crimson', 1200: 'royalblue', 1500: 'crimson',
           4000: 'crimson', 5000: 'royalblue', 6000: 'orange'}

linestyles = {900: ':', 1350: '--', 1800: '-', 1200: '--', 1500: '-',
           4000: ':', 5000: '--', 6000: '-'}

hatches = {900: '\\\\\\', 1350: '///', 1800: '|||', 1200: '///', 1500: '|||',
           4000: '|||', 5000: '///', 6000: '\\\\\\'}
...
for network, values in perf.items():
    for comparison, performances in values.items():
        for performance in performances:
            means_llim = [performance.means[ind] - performance.std[ind] for ind in range(len(performance.means))]
            means_ulim = [performance.means[ind] + performance.std[ind] for ind in range(len(performance.means))]
            plt.plot(samples, performance.means, color=colours[float(performance.demand.split()[0])],
              linestyle=linestyles[float(performance.demand.split()[0])], alpha=0.8, label=performance.demand)
            plt.fill_between(samples, means_llim, means_ulim, alpha=0.3, hatch=hatches[float(performance.demand.split()[0])], edgecolor=colours[float(performance.demand.split()[0])], facecolor=colours[float(performance.demand.split()[0])])
